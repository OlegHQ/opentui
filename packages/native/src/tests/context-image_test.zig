...
test "Context checked image creation retains bindings and publishes fresh pixel identities" {
    ...
    const value = try owner.raw().getImage(source);
    ...
    try testing.expectEqual(value, try owner.raw().getImage(retained));
    ...
    try testing.expectError(error.StaleHandle, owner.raw().getImage(source));
...
test "Context checked image decode transform and raw transfer use bounded owned storage" {
    ...
    try testing.expectError(error.StaleHandle, owner.raw().getImage(transformed));
...
test "Context checked image lazy decode failures preserve handles and lease accounting" {
    ...
    failing.fail_index = std.math.maxInt(usize);
    try testing.expectEqual(@as(usize, 0), (try owner.raw().getImage(decoded)).pixels.len);
