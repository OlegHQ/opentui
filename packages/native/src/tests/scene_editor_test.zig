...
fn expectLayoutDirty(owner: *context.Context, node: context.Handle, expected: u32) !void {
    ...
    try yoga.check(yoga.yogaNodeIsDirtyChecked((try owner.raw().getRenderable(node)).yoga_node, &dirty));
...
test "Scene editor paint-only updates preserve intrinsic layout" {
    ...
    try (try owner.raw().getRenderable(dependent)).setMeasureTarget(.{ .editor_view = (try owner.raw().getEditorView(fixture.view)).view });
    ...
    const state = (try owner.raw().getRenderable(fixture.node)).scene_node.?.owner;
    ...
    try testing.expectEqual(@as(u32, 1), (try owner.raw().getEditorView(fixture.view)).view.getPrimaryCursor().offset);
    try expectLayoutDirty(owner, fixture.node, 0);
    try expectLayoutDirty(owner, dependent, 0);
...
test "Scene editor retained geometry refreshes cursor following and shared buffer views" {
    ...
    try testing.expectEqual(@as(u32, 2), (try owner.raw().getEditorView(peer)).view.getViewport().?.y);
    try testing.expect(!state.preparation_dirty);
...
test "Scene editor custom self preserves cursor maintenance through hook replacement and completion" {
    ...
    (try owner.raw().getEditorView(fixture.view)).view.setWrapMode(.char);
    ...
    for ([_][2]u32{ .{ 32, 32 }, .{ 48, 16 }, .{ 48, 48 } }, 0..) |hooks, index| {
        ...
        try testing.expect((try owner.raw().getSession(fixture.session)).scene.?.preparation_dirty);
        _ = try owner.editorSelect(fixture.view, .{ .operation = .set, .start = 0, .end = 1 });
        try owner.editorSetTabColor(fixture.view, .{ 10, 20, 30, 255 });
        try testing.expect((try owner.raw().getSession(fixture.session)).scene.?.preparation_dirty);
...
test "Scene editor cursor follows focus visibility options and resource lifetime" {
    ...
    try (try owner.raw().getEditBuffer(fixture.edit)).buffer.setCursor(0, 1);
    try repaint(owner, fixture.session, background, true, 0);
    ...
    try repaint(owner, fixture.session, background, true, 0);
    try testing.expect(!(try owner.sceneGetCursorState(fixture.session)).visible);
    const empty = (try owner.raw().getSessionRenderer(fixture.session)).getNextBuffer().get(0, 0).?;
    ...
    try testing.expect(!cursor.visible);
    try testing.expect((try owner.raw().getRenderable(fixture.node)).scene_node.?.editor == null);
}
...
test "Scene editor budgeted paint prepares a rebound view without host drawing" {
    ...
    (try owner.raw().getEditorView(replacement)).view.setWrapMode(.char);
