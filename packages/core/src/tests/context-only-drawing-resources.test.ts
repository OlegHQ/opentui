...
test.each(["raw", "throw"] as const)(
  ...
  async (mode) => {
    ...
    const acquire = spyOn(lib, "sceneFrameAcquireBufferLease")
    const release = spyOn(lib, "contextReleaseBufferLease")
    const errors: Error[] = []
    const failure = new Error("raw paint failure")
    let saved: { char: Uint32Array } | undefined
